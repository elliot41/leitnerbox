    ...
    if (this.question && this.answer) {
      try {
        ...
        if (status === 201) {
          ...
          if (data && data[0]) {
            this.dispatchEvent(
              new CustomEvent("flashcard-added", {
                detail: data[0],
              })
            );
          }
          ...
          this.errorMsg =
            "Impossible d'ajouter la flashcard. Vérifiez la connexion à Supabase et les permissions.";
        }
      } catch (error) {
        this.errorMsg =
          "Erreur lors de l'ajout de la flashcard: " + error.message;
      }
    }
  }
