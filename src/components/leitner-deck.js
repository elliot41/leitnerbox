...
export class LeitnerDeck extends LitElement {
  ...
  async handleCorrect() {
    ...
    if (card) {
      ...
      this.dispatchEvent(new CustomEvent('flashcard-updated', {
      ...
      <div>
        ...
        <div class="box-nav">
          ${boxes.map(box => html`
            ...
            >
              Boîte ${box} (${this.flashcards.filter(fc => fc.box === box).length})
            </button>
          `)}
        </div>
            ...
            html`
              <div class="card-container">
                ...
                <div class="card-content">
                  <div class="question">
                    <strong>Question :</strong> ${cardsInBox[this.currentIdx].question}
                  </div>
                  
                  ${this.showAnswer ? 
                    html`
                      <div class="answer">
                        <strong>Réponse :</strong> ${cardsInBox[this.currentIdx].answer}
                      </div>
                      
                      <div class="actions">
                        <button class="correct-btn" @click=${this.handleCorrect}>
                          Bonne réponse
                        </button>
                        <button class="incorrect-btn" @click=${this.handleIncorrect}>
                          Mauvaise réponse
                        </button>
                      </div>
                    ` : 
                    html`
                      <button class="show-btn" @click=${() => this.showAnswer = true}>
                        Afficher la réponse
                      </button>
                    `
                  }
